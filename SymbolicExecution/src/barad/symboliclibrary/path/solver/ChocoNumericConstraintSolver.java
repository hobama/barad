 ...
 */
public class ChocoNumericConstraintSolver implements NumericConstraintSolver {
	...
	public int max_int;
	private Logger log;
	private HashSet<String> inputVariables;
	private LinkedHashMap<String, String> concretized;
	...
	public ChocoNumericConstraintSolver() {
		log = Logger.getLogger(this.getClass());
		inputVariables = new HashSet<String>();
		concretized = new LinkedHashMap<String, String>();
		min_double = -100;
		max_double = 100;
		max_int = 100;
		min_int = -100;
			...
			for (int i = 0; i < integerProblem.getNbIntVars(); i++) {
				...
				if (endIndex > -1) {
					...
					if (inputVariables.contains(variableName)) {
						concretized.put(variableName, String.valueOf(var.getVal()));
					}
				}
				if (DEBUG && VERBOSE) {
		...
		if (realProblemSolved) {
			for (int i = 0; i < realProblem.getNbRealVars(); i++) {
				...
				if (endIndex > -1) {
					String variableName = chocoVariableName.substring(0, endIndex);
					if (inputVariables.contains(variableName)) {
						concretized.put(variableName, String.valueOf(var.getValue()));
					}
				}
				if (DEBUG && VERBOSE) {
					log.info("Concretized variable\\value pair: " +  var.toString());
	 ...
	 */
	public RealConstraint getRealConstraint(IF_FCMPEQ if_fcmpeq) {
	 ...
	 * @return New Choco real constraint instance
	 */
	public RealConstraint getRealConstraint(IF_FCMPGE if_fcmpge) {
	 ...
	 */
	public RealConstraint getRealConstraint(IF_FCMPGT if_fcmpgt) {
	 ...
	 * @return New Choco real constraint instance
	 */
	public RealConstraint getRealConstraint(IF_FCMPLE if_fcmple) {
	 ...
	 * it is modeled as x + delta < = y
	 */
	public RealConstraint getRealConstraint(IF_FCMPLT if_fcmplt) {
		RealVar deltaVar = realProblem.makeRealVar(delta, delta);
	 ...
	 */
	public RealConstraint getRealConstraint(IF_FCMPNE if_fcmpne) {
		RealVar deltaVar = realProblem.makeRealVar(delta, delta);
	...
	private RealExp getRealExp(FloatInterface symbolicFloat) {
		...
		} else if (symbolicFloat instanceof IMUL) {
			return getRealExp((FMUL)symbolicFloat);
		} else/*(symbolicFloat instanceof IDIV)*/ {
	 ...
	 */
	public RealExp getRealExp(FADD fadd) {
	 ...
	 */
	public RealExp getRealExp(FSUB fsub) {
	 ...
	 */
	public RealExp getRealExp(FMUL fmul) {
	 ...
	 */
	public RealExp getRealExp(FDIV fdiv) {
	 ...
	 */
	public RealExp getRealExp(FCONST fconst) {
	 ...
	 */
	public RealExp getRealExp(FVAR fvar) {
		return realProblem.makeRealVar(fvar.getId(), min_double, max_double);
	}
	...
	*/
	...
	private IntExp getIntExp(IMUL imul) throws UnsupportedOperationByChoco {
		...
		try {
			if (imul.getOp1() instanceof ICONST && !(imul.getOp2() instanceof ICONST)) {
				intExp = integerProblem.mult(((ICONST)imul.getOp1()).getValue(), getIntExp(imul.getOp2()));
	...
	private IntExp getIntExp(IVAR ivar) {
		return integerProblem.makeBoundIntVar(ivar.getId(), min_int, max_int);
	}

	public void printConcretizedSolution() {
		log.info("Concrete solution:");
		for (Map.Entry<String, String> e: concretized.entrySet()) {
			//System.out.println("Variable: " + e.getKey() + " Value: " + e.getValue() + " Test: " + e.toString());
			log.info("Variable: " + e.getKey() + " Value: " + e.getValue());
		}	
	}
	...
	public static void main(String[] ars) {
		ChocoNumericConstraintSolver solver = new ChocoNumericConstraintSolver();
