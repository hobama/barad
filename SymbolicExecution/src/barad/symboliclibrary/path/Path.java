...
public class Path {
	...
	private static boolean reverseConstraints = false;
	...
	public static void createNewState() {
		if (reverseConstraints) {
			State newState = new State();
			log.info("State created: " + newState.getId());
			newState.getVariableValues().putAll(lastRemovedState.getVariableValues());
			newState.getFieldValues().putAll(lastRemovedState.getFieldValues());
			for (PathConstraintInterface pc: lastRemovedState.getConstraints()) {
				 newState.getConstraints().add(pc.inverse());
			}
	...
	public static void removeLastState() {
		...
		generateTestCase();
		lastRemovedState = stateStack.pop();
		log.debug("State removed: " + lastRemovedState.getId());
	}
