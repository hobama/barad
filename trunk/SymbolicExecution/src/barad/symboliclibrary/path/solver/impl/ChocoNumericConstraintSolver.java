...
public class ChocoNumericConstraintSolver implements NumericConstraintSolver {
	...
	private LinkedHashMap<String, Integer> concretizedInt;
	private LinkedHashMap<String, Double> concretizedReal;
	...
	public ChocoNumericConstraintSolver() {
		...
		concretizedInt = new LinkedHashMap<String, Integer>();
		concretizedReal = new LinkedHashMap<String, Double>();
	...
	public void addRealCosntraint(FloatPathConstraint constraint) {
		try {
			Constraint c = getRealConstraint(constraint);
			realProblem.post(c);
		} catch (UnsupportedOperationByChoco uobc) {
			log.error("Unsupported operation by Choco" + uobc, uobc);
		}
			...
			for (int i = 0; i < integerProblem.getNbIntVars(); i++) {
				...
				if (endIndex > -1) {
					...
					if (inputVariables.contains(variableName)) {
						concretizedInt.put(variableName, var.getVal());
					}
				}
				if (DEBUG && VERBOSE) {
			...
			for (int i = 0; i < realProblem.getNbRealVars(); i++) {
				...
				if (endIndex > -1) {
					...
					if (inputVariables.contains(variableName)) {
						...
						concretizedReal.put(variableName, var.getValue().getSup());
					}
				}
				if (DEBUG && VERBOSE) {
					log.info("Concretized variable\\value pair: " +  var.toString());
	...
	public void printConcretizedSolution() {
		...
		for (Map.Entry<String, Integer> e: concretizedInt.entrySet()) {
			log.info("Variable: " + e.getKey() + " Value: " + e.getValue());
		}
		for (Map.Entry<String, Double> e: concretizedReal.entrySet()) {
			log.info("Variable: " + e.getKey() + " Value: " + e.getValue());
		}
	}
	/**
