...
public class Path {
	...
	private static State lastRemovedState;
	static {
	...
	public static void createNewState() {
		...
		log.info("State created: " + newState.getId());
		if (stateStack.size() > 0) {
			newState.getVariableValues().putAll(stateStack.peek().getVariableValues());
			newState.getFieldValues().putAll(stateStack.peek().getFieldValues());
		}
	...
	public static void removeLastState() {
		...
		lastRemovedState = removed;
		log.debug("State removed: " + removed.getId());
	}
	...
	public static void reverseBranchConstraints() {
		...
		newState.getVariableValues().putAll(stateStack.peek().getVariableValues());
		newState.getFieldValues().putAll(stateStack.peek().getFieldValues());
		for (PathConstraintInterface pc: lastRemovedState.getConstraints()) {
			PathConstraintInterface inversed = pc.inverse(); 
			newState.getConstraints().add(inversed);
			log.debug("Added constraint: " + inversed.toString() + " to state: " + newState.getId());
		}
		stateStack.push(newState);
	}
	
	/**
	 * Generate a test suite for the current event handler
	 */
	...
	/**
	 * Sets the current state as termianl i.e. containing
	 * a return statement. Test cases for such states are
	 * not merged in the test reduction phase
	 */
	public static void setTerminal() {
		if (stateStack.size() > 0) {
			stateStack.peek().setTerminal(true);
		}
	}
