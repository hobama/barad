...
public class TestCase implements Comparable {
	...
	public boolean merge(TestCase other) {
		...
		if (!this.isTerminal() && !other.isTerminal() && !intersects(other)) {
	...
	/**
	 * Indicates if the test case is for a branch which
	 * ends with return statement. Such tests could not
	 * be merged
	 * @return True if the test is terminal
	 */
	...
	/**
	 * Sets if the test case is for a branch which
	 * ends with return statement. Such tests could not
	 * be merged
	 * @param True if the test is terminal
	 */
