	...
	*/
	...
	private void initGUI() {
		...
		}
		{
			...
			combo1.setItems(new java.lang.String[] {"Child","Student"});
		}
		{
	...
	private void button2WidgetSelected(SelectionEvent evt) {
       ...
       if (combo1.getText().trim().equals("")) {
           message = message + "Passenger Class\n";
           enteredAllData = false;
       }
       if (!(radioButton1.getSelection() || radioButton2.getSelection())) {
           message = message + "Aircompany\n";
           enteredAllData = false;
       }
       if (!enteredAllData) {
           return;
       ...
       int distanceRange = Math.abs(Integer.parseInt(text4.getText().trim()) - Integer.parseInt(text3.getText().trim()));
       String passengerClass = combo1.getText().trim();
       if (passengerClass.equals("Business")) {
      	 if (0 <= distanceRange && distanceRange < 50) {
      		 amountDue = 120 * coeficient;
           } else if (50 <= distanceRange && distanceRange < 60) {
          	 amountDue = 130 * coeficient;
           } else if (60 <= distanceRange && distanceRange < 70) {
          	 amountDue = 145 * coeficient;
           } else if (70 <= distanceRange && distanceRange < 80) {
          	 amountDue = 150 * coeficient;
           } else if (80 <= distanceRange && distanceRange < 100) {
          	 amountDue = 160 * coeficient;
           }
       } else if (passengerClass.equals("Child")) {
      	 if (0 <= distanceRange && distanceRange < 40) {
      		 amountDue = 100 * coeficient;
      	 } else if (40 <= distanceRange && distanceRange < 45) {
      		 amountDue = 110 * coeficient;
      	 } else if (45 <= distanceRange && distanceRange < 50) {
      		 amountDue = 120 * coeficient;
      	 } else if (50 <= distanceRange && distanceRange < 70) {
      		 amountDue = 140 * coeficient;
      	 } else if (70 <= distanceRange && distanceRange < 80) {
               amountDue = 150 * coeficient; 
           } else if (80 <= distanceRange && distanceRange < 85) {
          	 amountDue = 155 * coeficient;
           } else if (85 <= distanceRange && distanceRange < 100) {
          	 amountDue = 160 * coeficient;
           }   
       } else if (passengerClass.equals("Student")) {
      	 if (0 <= distanceRange && distanceRange < 20) {
      		 amountDue = 140 * coeficient;
      	 } else if (20 <= distanceRange && distanceRange < 60) {
      		 amountDue = 160 * coeficient;
      	 } else if (60 <= distanceRange && distanceRange < 65) {
      		 amountDue = 170 * coeficient;
      	 } if (65 <= distanceRange && distanceRange < 70) {
      		 amountDue = 160 * coeficient;
      	 } else if (70 <= distanceRange && distanceRange < 100) {
      	 ...
      	 if (0 <= distanceRange && distanceRange < 20) {
      		 ...
      		 amountDue = 110 * coeficient;
      	 } if (70 <= distanceRange && distanceRange < 100) {
